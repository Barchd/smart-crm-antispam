...
Поток обработки входящей заявки от приёма до создания CRM-сущностей.

```mermaid
sequenceDiagram
    participant SRC as Источник<br/>(сайт/форма)
    ...
    SRC->>API: HMAC-подписанный POST
    API->>API: Проверка HMAC, rate limit, honeypot
